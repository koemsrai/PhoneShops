<?php 
  ...
  if (isset($_POST['submit'])) {
    $bname = $_POST['bname']; 
    $sqlInsert = "INSERT INTO tbl_brand (name) VALUES ('$bname')";
   ...
   if(isset($_REQUEST ['BrandID'])){
    ...
    if(isset($_REQUEST['update'])){
        ...
        $sqlUpdate = "UPDATE tbl_brand SET `name` = '$bname' WHERE `id` = $BrandID";
    ...
    $frm = $myCon->query("SELECT * FROM `tbl_brand` WHERE id= $BrandID")->fetch_assoc();
}
else{
  $frm=["name"=>"",];
}
...
if(isset($_REQUEST['delID'])){
  ...
  $sqlDelete  = "DELETE FROM `tbl_brand` WHERE id=$delID";
    ...
    <div class="container mt-5">
      <form class="row" method="POST" action="">
        ...
        <div class="col-xl-10 col-11">
          <input type="text" class="form-control" name="bname" placeholder="Enter brand name"  value="<?php echo $frm['name']?>">
        </div>
        ...
        <?php
          if(isset($_REQUEST['BrandID'])){
              echo'
                ...
                <a href="brand.php"  class="btn btn-primary py-2 px-4 btn-sm">New</a>
              ';
            }
              else
                echo'
      ...
      <h4 class="mb-3 mt-5 1875rem">Brand List</h4>
              ...
              while($row = $qr->fetch_assoc()){
                echo'
                  <tr class="align-middle">
                    ...
                    <td>'.$row['name'].'</td>
                    <td>
                      <a href="brand.php?BrandID='.$row['id'].'" class="btn btn-primary btn-sm"> <i class="fa fa-edit"></i> </a>
                      <a href="#" data-href="brand.php?delID='.$row['id'].'"  data-bs-toggle="modal" data-bs-target="#confirmDelete" class="btn btn-danger btn-sm"><i class="fas fa-trash-alt"></i></a>
                    </td>
                  </tr>
                ';
                $i++;
